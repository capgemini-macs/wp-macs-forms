...
class Email extends Field {
	...
	public function render_field() {
		?>
			<div class="mf_field mf_field--email <?php echo esc_attr( $this->get_render_required_class() ); ?>" data-validate="email">
				<label for="<?php echo esc_attr( $this->id ); ?>" /><?php echo esc_html( $this->label ); ?>
					<?php echo wp_kses_post( $this->get_render_required_symbol() ); ?>
				</label>
				<input type="email" id="<?php echo esc_attr( $this->id ); ?>" class="mf_field__input empty" name="<?php echo esc_attr( $this->id ); ?>" <?php echo esc_attr( $this->get_render_required() ); ?>/>
			</div>
		<?php
	}
